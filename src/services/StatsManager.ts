...
import { getCharacterCount, getWordCount } from './StatsUtils';
...
export default class StatsManager { 
  ...
  async updateToday(): Promise<void> {
    ...
    const totalTimeSpent = await this.calcTotalTime();
    const totalXP = await this.calcTotalXp();
  ...
  public async change(text: string) {
    ...
    public async recalcTotals() {
      ...
      if (
        ...
        todayHist.totalTimeSpent = await this.calcTotalTimeSpent();
        this.update();
      } else {
        this.updateToday();
      }
